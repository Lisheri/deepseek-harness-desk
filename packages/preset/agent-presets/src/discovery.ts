...
export async function scanRoot(root: PresetRoot): Promise<AgentPreset[]> {
  ...
  try {
    ...
    // entries whose Dirent methods are missing, so directory-ness is checked
    // per candidate below instead.
  ...
  for (const name of children) {
    ...
    // lstat keeps Dirent.isDirectory's no-follow semantics: a symlinked
    // preset directory is skipped, like the previous withFileTypes check.
    let entryStat
    try {
      entryStat = await lstat(directory)
    } catch {
      // Deleted between listing and stat: nothing occupies the id anymore.
      continue
    }

...
The webview stays browser-grade: the shell registers no Tauri commands or plugins, so the GUI's HTTP/WebSocket transport and `window.__DSH_BOOT__` injection work exactly as in a browser. The one IPC edge is the desktop chrome added later — a Chinese native menu whose 文件 actions the shell forwards as `desktop-menu` events under the `core:event:default` capability, and the overlay titlebar the GUI draws itself ([desktop chrome note](2026-08-25-desktop-chrome-titlebar-menu.md)). The placeholder page under `desktop/ui/` shows while the server starts and reports startup failure or unexpected exit.
